...
> 논문을 주면, 결과가 실제로 재현되는지 알려줍니다.

**ReproRun**은 *"논문이 X를 주장한다"*에서 *"X가 실제로 내 컴퓨터에서 돌아간다"*까지의 고통스러운 여정을 자동화하는 [Claude Code](https://claude.com/claude-code) 커스텀 스킬입니다. 논문은 멋진 숫자를 내놓지만, 그것을 재현하는 일은 대개 망가진 코드, 빌드 불가능한 환경, 버전 불일치 속에서 좌초합니다. ReproRun은 전체 파이프라인을 처음부터 끝까지 처리합니다:

**논문 읽기 → 코드 및 데이터 찾기 → 환경 구축 → 스모크 테스트 → 전체 실행 → 측정된 숫자를 논문의 주장과 비교.**

---

## ✨ 모든 플랫폼에 맞춰 적응하도록 설계됨

ReproRun은 하나의 고정된 설정을 가정하는 대신, **무엇이 주어지든 스스로 적응하도록** 설계되었습니다:

- **크로스 OS** — Windows, macOS, Linux에서 실행
- **크로스 언어** — Python은 전체 파이프라인 지원; R / MATLAB / Julia는 최소 경로 지원
- **크로스 도메인** — 단일세포 생물학, 이미지 ML 등, 도메인별 재구성 없이 처리
- **크로스 모드** — *도구 모드*(`pip install` + 호출 스크립트 작성) 또는 *실험 모드*(저장소 클론 후 스크립트 실행) — 논문마다 자동 선택
...
ReproRun은 Claude Code 스킬입니다. 사용하려면:

1. [Claude Code](https://claude.com/claude-code)를 설치합니다.
2. `paper-reproduction/` 폴더를 Claude Code가 스킬을 로드할 수 있는 위치에 둡니다.
3. Claude Code 세션에서 그냥 요청하세요 — 예: *"reproduce scVelo"* 또는 *"reproduce Table 2 from this paper."* 스킬이 자동으로 트리거됩니다.
...
**v1.0.0** · 안정 (유지보수 모드) · Claude Code에서 유지보수
