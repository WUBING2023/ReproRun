...
**ReproRun** は、互換性のある任意の AI コーディングアシスタントから利用できる、ポータブルな **AI エージェントスキル** です。*「ある論文が X を主張している」* から *「X が実際に自分のマシンで動く」* までの苦痛に満ちた道のりを自動化します。論文は美しい数値を提示しますが、その再現はたいてい壊れたコード、ビルドできない環境、バージョンのずれによって頓挫します。ReproRun はパイプライン全体をエンドツーエンドで処理します。
...
ReproRun は、互換性のある任意の AI コーディングアシスタントで動作するエージェントスキルです。使い方は次のとおりです。

1. スキルを読み込める AI コーディングエージェントを使用します。
2. `paper-reproduction/` フォルダを、エージェントがスキルを読み込める場所に配置します。
3. セッションで、ただ頼むだけです — 例えば *「reproduce scVelo」* や *「この論文の Table 2 を再現して」* など。スキルは自動的に起動します。
