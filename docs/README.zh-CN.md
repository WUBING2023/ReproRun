...
**ReproRun** 是一个自定义的 [Claude Code](https://claude.com/claude-code) skill，它把从*"某篇论文声称 X"*到*"X 真的能在我的机器上跑起来"*这条痛苦的路径自动化了。论文给出的数字往往很漂亮，但复现时通常会死在坏掉的代码、装不起来的环境，以及版本漂移上。ReproRun 把整条流水线从头到尾全部接管：
...
ReproRun 是一个 Claude Code skill。使用方法：

1. 安装 [Claude Code](https://claude.com/claude-code)。
2. 把 `paper-reproduction/` 文件夹放到 Claude Code 能加载 skill 的位置。
3. 在 Claude Code 会话里直接提要求即可 —— 例如*"复现 scVelo"*或*"复现这篇论文的 Table 2"*。skill 会自动触发。
...
**v1.0.0** · 稳定（维护模式）· 在 Claude Code 上维护
